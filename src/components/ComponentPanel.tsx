...
const SortableLibraryComponent = ({
  component,
  onComponentClick,
}: SortableLibraryComponentProps) => {
...
const ComponentPanel = forwardRef<HTMLDivElement, ComponentPanelProps>(
  ({ onComponentClick, onClosePanel }, ref) => {
    ...
    const orderedComponents = useMemo(() => {
      ...
      const remaining = new Map(
        allComponentsArray.map((comp) => [comp.type, comp])
      );
    ...
    const handleSearchButtonClick = () => {
      ...
      if (!isSearchOpen) {
        setTimeout(() => {
          document
            .querySelector<HTMLInputElement>('.component-panel-search-input')
            ?.focus();
        }, 100);
      } else {
        setSearchTerm('');
    ...
    return (
      <div className="p-4 flex flex-col h-full" ref={ref}>
          ...
          <div>
            <div className="flex items-center mb-2 text-blue-700 dark:text-blue-300">
              ...
              <span className="text-sm font-medium">
                Drag to reorder, Click to add
              </span>
            </div>
            <p className="text-xs text-blue-600 dark:text-blue-400">
              Drag and drop components to change their order in the panel. Click
        ...
        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext
            items={filteredComponents.map(
              (component) => component.type as UniqueIdentifier
            )}
